...
from keyboards.default.users import phone_number
...
async def bot_start(message: types.Message):
        if await get_user(chat_id=message.chat.id):
            ...
            await message.answer(text=text, reply_markup=globalic_menu)
        else:
...
async def phone_handler(message: types.Message, state: FSMContext):
    ...
    await message.answer(text=text, reply_markup=chicco_main_menu)
    await state.finish()